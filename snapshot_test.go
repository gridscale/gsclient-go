...
func TestClient_GetStorageSnapshotList(t *testing.T) {
	...
	for _, test := range uuidCommonTestCases {
		res, err := client.GetStorageSnapshotList(context.Background(), test.testUUID)
		if test.isFailed {
			assert.NotNil(t, err)
		} else {
...
func TestClient_GetStorageSnapshot(t *testing.T) {
	...
	for _, testStorageID := range uuidCommonTestCases {
		for _, testSnapshotID := range uuidCommonTestCases {
			res, err := client.GetStorageSnapshot(context.Background(), testStorageID.testUUID, testSnapshotID.testUUID)
...
func TestClient_DeleteStorageSnapshot(t *testing.T) {
	for _, clientTest := range syncClientTestCases {
		...
		for _, serverTest := range commonSuccessFailTestCases {
			...
			for _, testStorageID := range uuidCommonTestCases {
				for _, testSnapshotID := range uuidCommonTestCases {
					err := client.DeleteStorageSnapshot(context.Background(), testStorageID.testUUID, testSnapshotID.testUUID)
					if testStorageID.isFailed || testSnapshotID.isFailed || isFailed {
						assert.NotNil(t, err)
					} else {
...
func TestClient_GetSnapshotsByLocation(t *testing.T) {
	...
	for _, test := range uuidCommonTestCases {
		res, err := client.GetSnapshotsByLocation(context.Background(), test.testUUID)
		if test.isFailed {
			assert.NotNil(t, err)
		} else {
...
func TestClient_GetDeletedSnapshots(t *testing.T) {
	...
	res, err := client.GetDeletedSnapshots(context.Background())
	assert.Nil(t, err, "GetSnapshotsByLocation returned an error %v", err)
	assert.Equal(t, 1, len(res))
...
func TestClient_waitForSnapshotActive(t *testing.T) {
	...
	for _, serverTest := range commonSuccessFailTestCases {
		...
		for _, isTimeoutTest := range timeoutTestCases {
			...
			err := client.waitForSnapshotActive(context.Background(), dummyUUID, dummyUUID)
...
func TestClient_waitForSnapshotDeleted(t *testing.T) {
	...
	for _, serverTest := range commonSuccessFailTestCases {
		...
		for _, isTimeoutTest := range timeoutTestCases {
			...
			for _, testStorageID := range uuidCommonTestCases {
				for _, testSnapshotID := range uuidCommonTestCases {
					err := client.waitForSnapshotDeleted(context.Background(), testStorageID.testUUID, testSnapshotID.testUUID)
